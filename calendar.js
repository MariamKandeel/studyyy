...
function formatDate(key){ const [y,m,day] = key.split("-").map(Number); return new Date(y,m-1,day).toLocaleDateString(undefined,{weekday:"short",month:"short",day:"numeric"}); }
...
function openModal(date=selectedDate){ selectedDate=date||todayKey; $("eventId").value=""; $("modalTitle").textContent="Add Deadline"; $("eventForm").reset(); $("eventDate").value=selectedDate; $("formError").textContent=""; $("eventModal").classList.add("active"); $("eventModal").setAttribute("aria-hidden","false"); $("eventTitle").focus(); }
function closeModal(){ $("eventModal").classList.remove("active"); $("eventModal").setAttribute("aria-hidden","true"); }
function editEvent(id){ const e=events.find(x=>String(x.id)===String(id)); if(!e) return; selectedDate=e.date; $("eventId").value=e.id; $("eventTitle").value=e.title; $("eventDate").value=e.date; $("eventTime").value=e.time||""; $("eventType").value=e.type; $("eventPriority").value=e.priority; $("eventNotes").value=e.notes||""; $("modalTitle").textContent="Edit Event"; $("formError").textContent=""; $("eventModal").classList.add("active"); $("eventModal").setAttribute("aria-hidden","false"); $("eventTitle").focus(); }
function deleteEvent(id){ const event=events.find(e=>String(e.id)===String(id)); if(!event) return; $("deleteEventId").value=event.id; $("deleteEventName").textContent=event.title||"this deadline"; $("deleteModal").classList.add("active"); $("deleteModal").setAttribute("aria-hidden","false"); }
function closeDeleteModal(){ $("deleteModal").classList.remove("active"); $("deleteModal").setAttribute("aria-hidden","true"); }
async function confirmDeleteEvent(){ const id=$("deleteEventId").value; try { await BlueFlowStore.removeEvent(currentUser,id); closeDeleteModal(); } catch(error){ console.error(error); alert("Could not delete the deadline. Please check your connection."); } }
...
$("eventForm").addEventListener("submit",async e=>{
  ...
  const data={id,title,date,time:$("eventTime").value,type:$("eventType").value,priority:$("eventPriority").value,notes:$("eventNotes").value.trim()};
  try { await BlueFlowStore.setEvent(currentUser,data); closeModal(); viewDate=new Date(`${date}T12:00:00`); render(); } catch(error){ console.error(error); $("formError").textContent="Could not save the event. Please check your connection."; }
});
...
$("closeDeleteModal").addEventListener("click",closeDeleteModal); $("cancelDeleteBtn").addEventListener("click",closeDeleteModal); $("confirmDeleteBtn").addEventListener("click",confirmDeleteEvent);
...
window.addEventListener("DOMContentLoaded",async()=>{
  ...
  await BlueFlowStore.seedEventsIfEmpty(currentUser);
  BlueFlowStore.watchEvents(currentUser,items=>{events=items;render();});
});
