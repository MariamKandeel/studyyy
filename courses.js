...
function renderCourses(allTasks) {
  ...
  grid.innerHTML = courses.map(course => {
    ...
    return `
      <article class="course-card">
        <div class="course-top"><div class="course-icon"><i class="fas ${course.icon}"></i></div><span class="course-code">${course.code}</span></div>
        <h3>${course.name}</h3>
        <p class="course-description">${course.description}</p>
        <div class="course-stats">
          <div class="course-stat"><strong>${total}</strong>Total Tasks</div>
          <div class="course-stat"><strong>${completed}</strong>Completed</div>
          <div class="course-stat"><strong>${pending}</strong>Pending</div>
        </div>
        <div class="course-progress"><div class="progress-fill" style="width: ${progress}%"></div></div>
        <div class="course-bottom"><span class="progress-label">Progress: <strong>${progress}%</strong></span><a class="view-course" href="course-todo.html?course=${course.id}">View To-Do <i class="fas fa-arrow-right"></i></a></div>
      </article>`;
  }).join("");
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!user) return;
  await BlueFlowStore.seedTasksIfEmpty(user, []);
  BlueFlowStore.watchTasks(user, renderCourses);
});
